...
export default class ImplicitGrantStrategy implements IAuthStrategy {
    ...
    public async getOrCreateAccessTokenWithState(state: string): Promise<AccessToken> {
        const token = await this.cache.getOrCreate<AccessToken>(
            ImplicitGrantStrategy.cacheKey,
            async () => {
    ...
    private async redirectOrVerifyTokenWithState(state: string): Promise<AccessToken> {
